...
class ModelsCategories extends Model {
    ...
    public function getAllProducts($param) {
        ...
        if (isset($param['page']) && is_numeric($param['page'])) {
            $this->pagination->page = $param['page'];
        } else {
    ...
    public function searchBooksByAuthors($param) {
        ...
        // pagination 
        $this->pagination->total = ($total->num_rows > 0) ? $total->row['total'] : 0;
    ...
    private function getBooksAuthor($author) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "book_to_author WHERE author_id = " . $author . "");
    ...
    private function getBook($book_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "books WHERE id = " . $book_id . "");

        return $query->row;
    }

    private function getAuthorsBook($book_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "book_to_author WHERE book_id = " . $book_id . "");
    ...
    private function getAuthor($author_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "authors WHERE id = " . $author_id . "");
    ...
    private function getCountProducts() {
        $query = $this->db->query("SELECT COUNT(*) as total FROM " . DB_PREFIX . "products");
